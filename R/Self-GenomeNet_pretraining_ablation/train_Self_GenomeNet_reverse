#this file is for ablation study of reverse-varying-length estimation
  ...
  function(path,
      ...
      for (b in seq(batches)) {
        with(tf$GradientTape() %as% tape, {
          ...
          if (isTRUE(Self_GenomeNet)) {
            a_complement <- tf$convert_to_tensor(array(as.array(a)[, (dim(a)[2]):1, ], dim = c(dim(a)[1], dim(a)[2], dim(a)[3])))
    ...
    val_loop <- function(batches = steps.per.epoch, epoch, Self_GenomeNet) {
      for (b in seq(ceiling(batches*0.1))) {  
        ...
        if (isTRUE(Self_GenomeNet)) {
              a_complement <- #a[, tf$convert_to_tensor(seq((dim(a)[2]),1)), tf$convert_to_tensor(seq(4,1))]
